...
class AddReactionModel {
  void discordReaction(
      String actionId, String message, String server, String channel, God god) {
    final url = Uri.parse('$urlPrefix/services/discord/reaction/message');
  ...
  void mailReaction(String actionId, String object, String content,
    ...
    final url = Uri.parse('$urlPrefix/services/mail/reaction/mail');
    final header = {"Authorization": "Bearer " + god.globalToken};
    String tmp = "";
    for (int i = 0; i < god.globalContainer.service.length; i++) {
    ...
    for (int i = 0; i < god.globalContainer.actionCodebasePush.length; i++) {
      if (god.globalContainer.actionCodebasePush[i].id == actionId) {
        god.globalContainer.actionCodebasePush[i].reactionMail.add(reaction);
      }
    }
    for (int i = 0;
        i < god.globalContainer.actionCodebaseTicketCreation.length;
    ...
    for (int i = 0; i < god.globalContainer.actionGithubIssue.length; i++) {
      if (god.globalContainer.actionGithubIssue[i].id == actionId) {
        god.globalContainer.actionGithubIssue[i].reactionMail.add(reaction);
      }
    }
    for (int i = 0; i < god.globalContainer.actionGithubLabel.length; i++) {
      if (god.globalContainer.actionGithubLabel[i].id == actionId) {
        god.globalContainer.actionGithubLabel[i].reactionMail.add(reaction);
      }
    }
    for (int i = 0; i < god.globalContainer.actionGithubMilestone.length; i++) {
      if (god.globalContainer.actionGithubMilestone[i].id == actionId) {
        god.globalContainer.actionGithubMilestone[i].reactionMail.add(reaction);
      }
    }
    for (int i = 0;
        i < god.globalContainer.actionGithubPullRequest.length;
    ...
    for (int i = 0; i < god.globalContainer.actionGithubPush.length; i++) {
      if (god.globalContainer.actionGithubPush[i].id == actionId) {
        god.globalContainer.actionGithubPush[i].reactionMail.add(reaction);
      }
    }
    for (int i = 0; i < god.globalContainer.actionGitlabComment.length; i++) {
      if (god.globalContainer.actionGitlabComment[i].id == actionId) {
        god.globalContainer.actionGitlabComment[i].reactionMail.add(reaction);
      }
    }
    for (int i = 0; i < god.globalContainer.actionGitlabIssue.length; i++) {
      if (god.globalContainer.actionGitlabIssue[i].id == actionId) {
        god.globalContainer.actionGitlabIssue[i].reactionMail.add(reaction);
      }
    }
    for (int i = 0; i < god.globalContainer.actionGitlabPush.length; i++) {
      if (god.globalContainer.actionGitlabPush[i].id == actionId) {
        god.globalContainer.actionGitlabPush[i].reactionMail.add(reaction);
      }
    }
    for (int i = 0;
        i < god.globalContainer.actionGitlabMergeRequest.length;
    ...
    for (int i = 0; i < god.globalContainer.actionGitlabWiki.length; i++) {
      if (god.globalContainer.actionGitlabWiki[i].id == actionId) {
        god.globalContainer.actionGitlabWiki[i].reactionMail.add(reaction);
      }
    }
    post(url, headers: header, body: body);
  }
  ...
  void twitterTweetReaction(String actionId, String message, God god) {
    final url = Uri.parse('$urlPrefix/services/twitter/reaction/tweet');
    final header = {"Authorization": "Bearer " + god.globalToken};
    String tmp = "";
    for (int i = 0; i < god.globalContainer.service.length; i++) {
  ...
  void twitterFollowReaction(String actionId, String user, God god) {
    final url = Uri.parse('$urlPrefix/services/twitter/reaction/follow');
    final header = {"Authorization": "Bearer " + god.globalToken};
    String tmp = "";
    for (int i = 0; i < god.globalContainer.service.length; i++) {
      if (god.globalContainer.service[i].name == "twitter") {
  ...
  void twitterRetweetReaction(String actionId, String tweetId, God god) {
    final url = Uri.parse('$urlPrefix/services/twitter/reaction/retweet');
    final header = {"Authorization": "Bearer " + god.globalToken};
    String tmp = "";
    for (int i = 0; i < god.globalContainer.service.length; i++) {
      if (god.globalContainer.service[i].name == "twitter") {
  ...
  void twitterLikeReaction(String actionId, String tweetId, God god) {
    final url = Uri.parse('$urlPrefix/services/twitter/reaction/like');
    final header = {"Authorization": "Bearer " + god.globalToken};
    String tmp = "";
    for (int i = 0; i < god.globalContainer.service.length; i++) {
      if (god.globalContainer.service[i].name == "twitter") {
